...
namespace AndroidCompound5
{
    ...
    {
        private const string TAG = "BluetoothPrintService";
        ...
        protected class ConnectThread : Thread
        {
            ...
            private BluetoothPrintService service;
            ...
            public ConnectThread(BluetoothDevice device, BluetoothPrintService service)
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "create() failed", e.Message, Enums.LogType.Error);
                }
                socket = tmp;
            ...
            public override void Run()
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "unable to connect()", e.Message, Enums.LogType.Error);
                    // Close the socket
                    try
                    {
                        socket.Close();
                    ...
                    {
                        LogFile.WriteLogFile(TAG, "unable to close() socket during connection failure", e2.Message, Enums.LogType.Error);
                    }
            ...
            public void Cancel()
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "Cancel() close() of connect socket failed", e.Message, Enums.LogType.Error);
                }
            }
        }
        ...
        private class ConnectedThread : Thread
        {
            ...
            public ConnectedThread(BluetoothSocket socket, BluetoothPrintService service)
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "ConnectedThread()", e.Message, Enums.LogType.Error);
                }
                inStream = tmpIn;
                outStream = tmpOut;
            ...
            public override void Run()
            {
                ...
                while (service.GetState() == STATE_CONNECTED)
                {
                    ...
                    {
                        LogFile.WriteLogFile(TAG, "Run()", e.Message, Enums.LogType.Error);
            ...
            public void Write(byte[] buffer)
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "Write()", e.Message, Enums.LogType.Error);
                }
            }
            ...
            public void Cancel()
            {
                ...
                {
                    LogFile.WriteLogFile(TAG, "Cancel()", e.Message, Enums.LogType.Error);
                }
            }
        }
    }
