...
namespace AndroidCompound5
{
	public static class CompoundAccess
	{
		...
		public static bool AddCompoundAccess(CompoundDto compoundDto)
		{
			try
			{
				DbContextProvider.Instance.Compounds.Add(compoundDto);
				DbContextProvider.Instance.SaveChanges();
				return true;
			}
			catch (Exception ex)
			{
				LogFile.WriteLogFile("Error WriteCompound CompNum : " + compoundDto.CompNum, Enums.LogType.Error);
				LogFile.WriteLogFile("Error WriteCompound Message : " + ex.Message, Enums.LogType.Error);
				return false;
			}
		}
		public static List<CompoundDto> GetCompoundAccess()
		{
			return [.. DbContextProvider.Instance.Compounds];
		}
	}
}