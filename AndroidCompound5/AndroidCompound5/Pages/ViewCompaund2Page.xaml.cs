...
public partial class ViewCompaund2Page : ContentPage
{
	string _stringCompoundNo;
	private CompoundDto _compoundDto;
	public ViewCompaund2Page(string compaundNo)
	{
		InitializeComponent();
	...
	private async Task SetInit()
	{
		...
		try
		{
			...
			else
			{
				...
				txtCarNum.Text = _compoundDto.Compound2Type.CarNum;
				txtRoadTax.Text = _compoundDto.Compound2Type.RoadTax;

				var category = TableFilBll.GetCarCategory(_compoundDto.Compound2Type.Category);
				txtCategory.Text =  category != null ? category.ShortDesc : "";
				txtMake.Text = _compoundDto.Compound2Type.CarTypeDesc;
				txtLocation.Text = _compoundDto.StreetDesc;

				txtTempatJadi.Text = _compoundDto.Tempatjadi;
				var delivery = TableFilBll.GetDeliveryByCode(_compoundDto.Compound2Type.DeliveryCode);
				txtSerah.Text = delivery != null ? delivery.ShortDesc : "";

				//txtMuatan.Text = _compoundDto.Compound2Type.Muatan;
