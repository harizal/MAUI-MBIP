...
using Constants = AndroidCompound5.AimforceUtils.Constants;
namespace AndroidCompound5
{
	public static class GeneralBll
	{
		...
		public static ConfigAppDto? GetConfig()
		{
			...
			return ConfigAccess.GetConfigAccess() ?? null;
		...
		public static List<CompDescDto> GetCompDescByActAndOfdCode(string act, string ofdCode)
		{
			...
			return [.. CompDescAccess.GetCompDescAccess().Where(c => c.ActCode == act && c.OfdCode == ofdCode)];
		}
...
*/
		...
		{
			string path = GeneralAndroidClass.GetExternalStorageDirectory() + Constants.ProgramPath +  Constants.DatabaseName;
			return path;
		}
		...
		internal static void InitDatabase()
		{
			...
			string GetFullPath(string fileName)
			{
				return System.IO.Path.Combine(
					GeneralAndroidClass.GetExternalStorageDirectory(),
					Constants.ProgramPath,
					Constants.MasterPath,
					fileName
				);
			}
			...
			if (!db.CompDesc.Any())
			{
				var path = GetFullPath(Constants.CompDesc);
				if (File.Exists(path))
				{
					var compDescList = CompDescAccess.GetCompDescAccess(path);
					db.CompDesc.AddRange(compDescList);
				}
			}

			if (!db.ConfigApp.Any())
			{
				var config = new ConfigAppDto
				{
					ServiceUrl = "https://1.9.46.172:38080/mbipwebservice/servicecompound.asmx",
					ServiceUPSBUrl = "http://175.138.109.52:9911/cmbparking/calc",
					ServiceAppletUrl = "https://smartparking.southmax.com.my/southmaxws/module-servlet",
					UrlPhoto = "http://1.9.46.172:38081/mdppwsparkingphoto/ws-servlet/WSModule/uploadPhoto?",
					UrlImage = "http://1.9.46.172:38082/home/search?",
					ServiceKey = "YWltZm9yY2Vzb2Z0d2FyZXNkbmJoZHdlYnNlcnZpY2VzY3JlZGVudGlhbHNAOTAwMDAzODM5MDI4",
					FtpHost = "192.168.8.1",
					FtpUser = "AIMFORCE",
					FtpPassword = "78742833",
					FtpUnload = "\\UNLOAD",
					...
					ImgsHeight = 300,
					GpsInterval = 1,
					SendCompoundInterval = 1,
					ServiceUser = "aimforce",
					ServicePassword = "1234",
					ApkFileName = "MBIPAndroid.MBIPAndroid.apk",
					CompanyID = "1234",
					ParkCode = "PK002",
					AppID = "ny35fmkt",
					VideoMaxDuration = 0,
					VideoMaxFileSize = 0,
					ApkFtpUrl = "ftp://1.9.46.170:521/"
				};
				db.ConfigApp.Add(config);
			}

			if (!db.Compounds.Any())
			{
				var path = GetFullPath(Constants.CompoundFil);
				if (File.Exists(path))
				{
					var compoundDtos = CompoundAccess.GetCompoundAccess(path);
					db.Compounds.AddRange(compoundDtos);
				}
			}

			if (!db.Enforcers.Any())
			{
				var path = GetFullPath(Constants.EnforcerFil);
				if (File.Exists(path))
				{
					var enforcers = EnforcerAccess.GetEnforcerAccess(path);
					db.Enforcers.AddRange(enforcers);
				}
			}

			if (!db.LogServers.Any())
			{
				var path = GetFullPath(Constants.ENQUIRYLOGFIL);
				if (File.Exists(path))
				{
					var logServers = EnquiryLogAccess.GetEnquiryLogAccess(path);
					db.LogServers.AddRange(logServers);
				}
			}

			if (!db.Gps.Any())
			{
				var path = GetFullPath(Constants.GpsFil);
				if (File.Exists(path))
				{
					var gps = GpsAccess.GetGPSAccess(path);
					db.Gps.AddRange(gps);
				}
			}
			...
			if (!db.Handhelds.Any())
			{
				var path = GetFullPath(Constants.HandheldFil);
				if (File.Exists(path))
				{
					var handhelds = HandheldAccess.GetHandheldAccess(path);
					db.Handhelds.AddRange(handhelds);
				}
			}

			if (!db.Infos.Any())
			{
				var path = GetFullPath(Constants.InfoDat);
				if (File.Exists(path))
				{
					var infos = InfoAccess.GetInfoAccess(path);
					db.Infos.Add(infos);
				}
			}
			...
			if (!db.Messages.Any())
			{
				var path = GetFullPath(Constants.MessageFil);
				if (File.Exists(path))
				{
					var messages = MessageAccess.GetMessageAccess(path);
			...
			if (!db.Notes.Any())
			{
				var path = GetFullPath(Constants.NoteFil);
				if (File.Exists(path))
				{
					var notes = NoteAccess.GetNoteAccess(path);
			...
			if (!db.Offrates.Any())
			{
				var path = GetFullPath(Constants.OffRateFil);
				if (File.Exists(path))
				{
					var offerates = OffrateAccess.GetOffRateAccess(path);
					db.Offrates.AddRange(offerates);
				}
			}

			if (!db.PassBulans.Any())
			{
				var path = GetFullPath(Constants.PassFil);
				if (File.Exists(path))
				{
					var pasbulans = PassBulanAccess.GetPassBulanAccess(path);
					db.PassBulans.AddRange(pasbulans);
				}
			}

			if (!db.SemakPasses.Any())
			{
				var path = GetFullPath(Constants.SEMAKPASSFIL);
				if (File.Exists(path))
				{
					var semakPass = SemakPassAccess.GetSemakPassAccess(path);
					db.SemakPasses.AddRange(semakPass);
				}
			}

			if (!db.Streets.Any())
			{
				var path = GetFullPath(Constants.StreetFil);
				if (File.Exists(path))
				{
					var streets = StreetAccess.GetStreetAccess(path);
			...
			if (!db.CarCategories.Any())
			{
				var path = GetFullPath(Constants.TableFil);
				if (File.Exists(path))
				{
					var carCategories = TableFilAccess.GetCarCategoryAccess(path);
					db.CarCategories.AddRange(carCategories);
				}
			}
			...
			if (!db.CarTypes.Any())
			{
				...
				if (File.Exists(path))
				{
					var carTypes = TableFilAccess.GetCarTypeAccess(path);
					db.CarTypes.AddRange(carTypes);
				}
			}
			...
			if (!db.CarColors.Any())
			{
				...
				if (File.Exists(path))
				{
					var carColors = TableFilAccess.GetCarColorAccess(path);
					db.CarColors.AddRange(carColors);
				}
			}
			...
			if (!db.Offends.Any())
			{
				...
				if (File.Exists(path))
				{
					var offends = TableFilAccess.GetOffendAccess(path);
					db.Offends.AddRange(offends);
				}
			}
			...
			if (!db.Zones.Any())
			{
				...
				if (File.Exists(path))
				{
					var zones = TableFilAccess.GetZoneAccess(path);
			...
			if (!db.Acts.Any())
			{
				...
				if (File.Exists(path))
				{
					var acts = TableFilAccess.GetActAccess(path);
			...
			if (!db.Deliveries.Any())
			{
				...
				if (File.Exists(path))
				{
					var deliveries = TableFilAccess.GetDeliveryAccess(path);
			...
			if (!db.Mukims.Any())
			{
				...
				if (File.Exists(path))
				{
					var mukims = TableFilAccess.GetMukimAccess(path);
					db.Mukims.AddRange(mukims);
				}
			}

			if (!db.TempatJadis.Any())
			{
				var path = GetFullPath(Constants.TableFil);
				if (File.Exists(path))
				{
					var tempatJadis = TableFilAccess.GetTempatJadiAccess(path);
					db.TempatJadis.AddRange(tempatJadis);
				}
			}
